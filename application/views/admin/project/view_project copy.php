                                ...
                                <?php if (!empty($task_list)) {
                                  ...
                                  foreach ($task_list as $key_a => $value_tk) { ?>
                                    <tr data-task-id="<?php echo $value_tk['task_id']; ?>">
                                      ...
                                      <td>
                                        <?php if ($value_tk['task_status'] !== '5') { ?> <input type="button" class="btn btn-success input-sm start_time" id="start_time" value="Start Work" onclick="startTime('<?php echo $value_tk['task_id']; ?>')">
                                          ...
                                          <input type="button" class="btn btn-success input-sm hold_time" id="hold_time" style="background-color: orange;display: none;" onclick="holdTime('<?php echo $value_tk['task_id']; ?>')" value="Hold">
                                          <div><span id="time"></span> </div><?php } else {
                                                                              echo "Task Completed";
                                                                            } ?>
                                      </td>
...
<div class="modal fade" id="taskstatusModal" tabindex="-1" aria-hidden="true">
      ...
      <div class="modal-body">
        ...
        <form enctype="multipart/form-data">
          <div class="row">
            <div class="col-sm-12 mt-3">
              <div class="form-group">
                <label for="companyName"> Task Status</label>
                <select class="form-control" id="task_status_t">
                  <?php
                  foreach ($work_status as $k => $val) {
                  ?>
                    <option value="<?php echo $val['tws_id']; ?>" <?php if ($value_tk['task_status'] == $val['tws_id']) {
            ...
            <div class="col-sm-12 mt-3">
              <div class="form-group">
                ...
                <input type="text" name="stopReason" id="stopReason" class="form-control">
  ...
  <div class="modal fade" id="valErrorModal" tabindex="-1" aria-hidden="true">
    ...
    <script type="text/javascript">
      ...
      // function validate_edit_discussion(ele) {//alert("11");
      // 	//hide_message_box(ele);
      // 	var hasError=0;
      // 	var  edit_discussion_subject = $("#edit_discussion_subject").val();

      //    if(edit_discussion_subject.trim()=='') { 
      //          // showError("Please enter  project name", " project_name");
      //          $('#edit_discussion_subject_msg').text('Please enter discussion subject');
      //            hasError = 1; } else {
      //             $('#edit_discussion_subject_msg').text(''); // Clear error if valid
      //         }
      ...
      function holdTime(task_id) {
        // alert("11"); alert(task_id);
        // var task_timerId = JSON.parse(localStorage.getItem('testObject'));
        // var task_log_id = task_timerId.data.task_log_id; // assuming the structure of the data is correct
        // alert(task_log_id);
