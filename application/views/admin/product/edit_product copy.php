                  ...
                  <div class="row">
                    ...
                    <div class="col-sm-12  mt-3">
                      ...
                      <?php $row_no = 1; ?>
                          ...
                          <?php
                            $product_id = (int) $product_data[0]['product_id']; // Cast to int to prevent injection
                          ...
                          <?php
                                                // $unit_size = '0';
                                                // if (!empty($product_data[0]['unit_size'])) {
                                                //     $unit_size = explode(",", $product_data[0]['unit_size']);
                                                // }
                                                // if (!empty($product_data[0]['item_type'])) {
                                                //     $item_type = explode(",", $product_data[0]['item_type']);
                                                // }
                                                // if (!empty($product_data[0]['item_category'])) {
                                                //     $item_cat = explode(",", $product_data[0]['item_category']);
                                                // }
                                           
                                                // if (!empty($product_data[0]['price_range'])) {
                                                //     $price_range = explode(",", $product_data[0]['price_range']);
                                                // }
                                                // if (!empty($product_data[0]['avg_rate'])) {
                                                //     $avg_rate = explode(",", $product_data[0]['avg_rate']);
                                                // }
                                                ?>
                          <tbody>
                          <?php if(!empty($price_range)){ for ($i = 0; $i < count($price_range); $i++) { ?>
                            <tr class="row_no" id="row_no_<?php echo $i; ?>">
                              <td>
                              <select class="form-control" id="item_type" name="item_type[<?php echo $i; ?>]">
                                                    <option value="">Select Type Of service </option>
                                                    <?php  if(!empty($item_type_list)){ foreach ($item_type_list as $key => $value) { ?>
                                                        <option value="<?php echo $value['item_type_id']; ?>" <?php if ($item_type[$i] == $value['item_type_id']) {
                                                        echo 'selected';
                                                    } ?> > <?php echo $value['item_type_name']; ?></option>
                                                        <?php }} ?>
                                                </select>
                              <td>
                         
                                                <select class="form-control" id="item_category" name="item_category[<?php echo $i; ?>]">
                                                    <option value="">Select Service Category </option>
                                                    <?php  if(!empty($item_category1 )){ foreach ($item_category1 as $key => $value) { ?>
                                                        <option value="<?php echo $value['item_category_id']; ?>" <?php if ($product_data[0]['item_category'] == $value['item_category_id']) {
                                                        echo 'selected';
                                                    } ?> > <?php echo $value['item_category_name']; ?></option>
                                                        <?php }} ?>
                                                </select>
                              </td>
                              <td>
                                <input type="text" class="form-control" name="unit_size[<?php echo $i; ?>]" id="unit_size" value="<?php echo isset($unit_size[$i]) ? $unit_size[$i] : ''; ?>"> 
                              </td>
                              <td>
                                <input type="number" class="form-control" name="price_range[<?php echo $i; ?>]" id="price_range" value="<?php echo isset($price_range[$i]) ? $price_range[$i] : ''; ?>">
                              </td>
                              <td>
                                <input type="number" class="form-control" name="avg_rate[<?php echo $i; ?>]" id="sale_price" value="<?php echo isset($avg_rate[$i]) ? $avg_rate[$i] : ''; ?>">
                              </td>
                              <td>
                                <button type="button" class="btn btn-danger" onclick="removeBanner(<?php echo $i; ?>)">
                          ...
                          <?php if(!empty($unit_size))
                                        {
                                            $unit_size_count = count($unit_size);
                                        }
                                        else{
                                            $unit_size_count = '0';
                                        }
                                    ?>
                                    <input type="hidden" name="rowNo" id="rowNo" value="<?php echo $unit_size_count; ?>"> 
                          <tfoot>
                            <tr>
                              <td colspan="5"></td>
                              <td>
...
function addFeature()
  {
    var row_no = $('.row_no').length+1;
    // alert(row_no);
    var tr='<tr class="row_no" id="row_no_'+row_no+'">'+

            '<td>'+
            '<select class="form-control" id="item_type" name="item_type['+row_no+']">'+
                '<option value="">Select Type Of service</option>'+
                '<?php if(!empty($item_type_list)){ foreach ($item_type_list as $key => $value) { ?>'+
            ...
            '</td>'+
             '<td>'+
            '<select class="form-control" id="item_category" name="item_category['+row_no+']">'+
            ...
            '<td>'+
            '<div class="form-group">'+
                '<input type="number" class="form-control" name="price_range['+row_no+']" id="price_range">'+
            '</div>'+
            '</td>'+
            '<td>'+
            '<div class="form-group">'+
                '<input type="number" class="form-control" name="avg_rate['+row_no+']" id="avg_rate">'+
            '</div>'+
            '</td>'+
            '<td>'+
            '<div class="form-group">'+
                    ...
                    <div class="col-sm-12  mt-3">
                      ...
                      <?php if (!empty($product_data[0]['image_name'])) {
                              ...
                              foreach ($images as $image) { ?>
                                  <img src="<?php echo base_url(); ?>upload/products/<?php echo trim($image); ?>" width="100" height="100">
                              <?php } 
                          } else { ?>
 ...
 var form_data = new FormData(); 
     ...
     $.ajax({
                   ...
                   var res=JSON.parse(response);
                    ...
                    setTimeout(function(){ $('#sub_msg').text(''); }, 3000);
...
var form_data = new FormData(); 
      ...
      $.ajax({
                    ...
                    success: function(response){
                     ...
                     $('#hsn_code_m').val('');
                     setTimeout(function(){ $('#msg_hsn').text(''); }, 3000);
  ...
  var form_data = new FormData(); 
      ...
      $.ajax({
        ...
        success: function(response){
         ...
         $('#unit').val('');
         $('#abbrivation').val('');
         setTimeout(function(){ $('#msg_unit').text(''); }, 3000);
    ...
    var form_data = new FormData(); 
        ...
        $.ajax({
                      ...
                      var res=JSON.parse(response);
                       ...
                       setTimeout(function(){ $('#child_msg1').text(''); }, 3000);
