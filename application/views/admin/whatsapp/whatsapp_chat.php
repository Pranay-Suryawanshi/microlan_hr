...
<body>
            ...
            <div class="header d-flex justify-content-between align-items-center" style="background-color: white">
                ...
                foreach ($res_user->result() as $val) {
                    ...
                    if (!empty($val->profile_photo)) {
                        $profile = base_url() . 'uploads/profile/' . $val->profile_photo;
                    } else {
                        ...
                        if ($cnt_res > 0) {
                            if ($user['user_flag'] == 1) {
                                if (!empty($user['profile_photo'])) {
                                    $profile = base_url() . 'uploads/profile/' . $user['profile_photo'];
                                } else {
                                    $profile = base_url() . 'assets/images/user1.jpg';
                                }
                            ...
                            ?>
                                ...
                                ?>
                                        <li style="display:none;" class="contact-box" onclick="handleClick('<?php echo $user['company_name']; ?>', 
                                            ...
                                            <div class="contact-details">
                                                <h6><?php echo $user['role_name']; ?>
                                                    <span class="text-muted adv_right"><?php echo $chatdate; ?></span>
                                                </h6>
                                                <p class="GroupName"><?php echo $user['user_name']; ?> </p>
                                                <!-- <span class="adv_color adv_right">3</span> -->
                                            </div>
                                    ...
                                    ?>
                                        ...
                                        <li class="contact-box" onclick="handleClick('<?php echo $user['company_name']; ?>', 
                                                        '<?php echo $user['user_name']; ?>' , '<?php echo $user['op_user_id']; ?>' 
                                                        , '<?php echo $user['mapping_plan_id']; ?>', '<?php echo $user['audit_status']; ?>', '<?php echo $user['role_name']; ?>'
                                                        , '<?php echo $profile ?>')">
                                            <img src="<?php echo $profile; ?>" alt="Contact Image" class="border_deactive_red" />
    ...
    <script>
        ...
        function handleClick1(organisation, subscription, opuserId, mapping_plan_id, audit_status, role_name, profile) 
        {
            //alert(opuserId);
            $(".MessageContainer").empty();
            
            document.getElementById("opuserId").value = opuserId
            document.getElementById("mapping_plan_id").value = mapping_plan_id
            document.getElementById("msgType").value = "Single";
            document.getElementById("MainChat").innerHTML = subscription + " - " + role_name;
            document.getElementById("MainChat1").innerHTML = "";
            document.getElementById("GroupChat").innerHTML = "";
            document.getElementById('MainChatImg').style.display = 'block';
            document.getElementById("MainChatImg").src = profile;
            document.getElementById('MainGroupImg').style.display = 'none';

            var element = document.getElementById('MainChatImg');
            element.setAttribute('data-target', 'updateImageModal'+opuserId);

            element.onclick = function() {
                $('#updateImageModal'+opuserId).modal('show');
            };

            //  alert("Clicked on " + organisation + " - " + subscription);
            // Your script here, for example:
            console.log("Clicked on " + organisation + " - " + subscription);
            ...
            $.ajax({
                type: 'POST',
                url: "<?php echo base_url(); ?>Whatsapp/get_chat",
                data: requestData,
                success: function(response) {
                    console.log(response);
                    if (response.success == 'success') {
        ...
        function handleClick(organisation, subscription, opuserId, mapping_plan_id, audit_status, role_name, profile) {
            // alert(profile);
            $(".MessageContainer").empty();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success == 'success') {
                        ...
                        updateChatMessages(response.chatData);
                        // Use the function to update chat messages
                        //pdateChatMessages(response.chatData);
        ...
        function updateChatMessages(chatData) {
            ...
            chatData.forEach(function(message) {
                ...
                // Set the trip_number as the card title
                $(".card-title").text("School-Auditor Chat");

                if (message.send_to_user == opuserId) {
                    // For driver_report messages
                    messageHtml += '<div class="message me">';
                    messageHtml += '<p class="messageContent">' + message.user_chat_message + '</p>';
                    messageHtml += '<div class="messageDetails">';
                    messageHtml += '<div class="messageTime">' + message.user_chat_added_date + '</div>';
                    messageHtml += ' <i class="fa fa-check"></i>';
                    messageHtml += '</div>';
                    messageHtml += '</div>';
                }
                if (message.user_chat_send_by == opuserId) {
                    // For manager_response messages
                    messageHtml += '<div class="message you">';
                    messageHtml += '<p class="messageContent">' + message.user_chat_message + '</p>';
                    messageHtml += '<div class="messageDetails">';
                    messageHtml += '<div class="messageTime">' + message.user_chat_added_date + '</div>';
                    messageHtml += ' <i class="fa fa-check"></i>';
